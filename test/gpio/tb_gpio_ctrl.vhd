        ...
        if run("basic") then
            ...
            for i in 0 to 1 loop
                ...
                for j in 0 to 3 loop
                    ...
                    i_pb_vector(j) <= '1';
                    wait_clock(G_DEBOUNCE_LIMIT + 1, clk_period);
                    check(o_sig_gen_src_sel(j) = '1', "Expected SigGen source " & integer'image(j) & " to be active");
                end loop;
            end loop;
            i_pb_vector  <= x"0";
            i_dip_vector <= 4b"0"; -- some vhdl-2008 syntax
            wait_clock(G_DEBOUNCE_LIMIT + 1, clk_period);
            check(o_sig_gen_src_sel = (o_sig_gen_src_sel'range => '0'), "Expected SigGen sources to be all zeros");
            ...
            check(o_ema_en = '0', "Expected off : ema");
